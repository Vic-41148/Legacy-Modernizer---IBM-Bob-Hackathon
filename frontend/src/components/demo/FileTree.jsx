...
function FileTree({ files, selectedFile, onFileSelect }) {
  ...
  const renderTree = (node, level = 0) => {
    ...
    return Object.values(node).map((item) => {
      ...
      if (item.isFile) {
        const hasChanges = item.file?.status === 'completed' || item.file?.changes;
        ...
        return (
          <div
            ...
            className={`
              file-tree-item
              ${isSelected ? 'selected' : ''}
              ${hasChanges ? 'bg-success-50 border-l-2 border-success-500' : ''}
            `}
            style={{ paddingLeft: `${(level + 1) * 12}px` }}
          >
            <File className={`w-4 h-4 flex-shrink-0 ${hasChanges ? 'text-success-600' : 'text-gray-500'}`} />
            <span className={`flex-1 text-sm truncate ${hasChanges ? 'font-medium text-success-800' : ''}`}>{item.name}</span>
  ...
  return (
    <div className="overflow-y-auto flex-1 h-full min-h-[250px]">
      {files.length === 0 ? (
        ...
        <div className="space-y-0.5 animate-fade-in pb-4">
          {renderTree(tree)}
        </div>
      )}
    </div>
